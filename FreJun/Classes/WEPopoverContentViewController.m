...
- (id)initWithStyle:(UITableViewStyle)style {
    ...
    if ((self = [super initWithStyle:style])) {
        ...
        if ([self respondsToSelector:@selector(setPreferredContentSize:)]) {
            self.preferredContentSize = CGSizeMake(300, 3 * 44 - 1);
        } else {
            self.contentSizeForViewInPopover = CGSizeMake(300, 3 * 44 - 1);
        }
    }
    return self;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //cell.textLabel.text = obj.emailTitle;
    cell.textLabel.text = [[[NSUserDefaults standardUserDefaults] objectForKey:@"accounts"] objectAtIndex:indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    dataclass *obj = [dataclass getInstance];
    obj.emailTitle = [[[NSUserDefaults standardUserDefaults] objectForKey:@"accounts"] objectAtIndex:indexPath.row];
